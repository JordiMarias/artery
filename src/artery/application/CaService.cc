...
void CaService::indicate(const vanetza::btp::DataIndication& ind, std::unique_ptr<vanetza::UpPacket> packet)
{
	...
	if (cam && cam->validate()) {
		...
		int latitude = round(mVehicleDataProvider->latitude(), microdegree) * Longitude_oneMicrodegreeEast;
		int longitude = round(mVehicleDataProvider->longitude(), microdegree) * Latitude_oneMicrodegreeNorth;
