...
#include "artery/application/AoiCaService.h"
...
SpeedValue_t buildSpeedValue(const vanetza::units::Velocity& v)
{
	...
	if (v >= upper) {
		...
		speed = aoi::round(v, aoi::centimeter_per_second) * SpeedValue_oneCentimeterPerSec;
	}
	return speed;
}
...
vanetza::asn1::Cam createCooperativeAwarenessMessage(const VehicleDataProvider& vdp, uint16_t genDeltaTime)
{
	...
	basic.referencePosition.longitude = aoi::round(vdp.longitude(), aoi::microdegree) * Longitude_oneMicrodegreeEast;
	basic.referencePosition.latitude = aoi::round(vdp.latitude(), aoi::microdegree) * Latitude_oneMicrodegreeNorth;
	...
	bvc.heading.headingValue = aoi::round(vdp.heading(), aoi::decidegree);
	bvc.heading.headingConfidence = HeadingConfidence_equalOrWithinOneDegree;
	bvc.speed.speedValue = aoi::buildSpeedValue(vdp.speed());
	...
	bvc.yawRate.yawRateValue = aoi::round(vdp.yaw_rate(), aoi::degree_per_second) * YawRateValue_degSec_000_01ToLeft * 100.0;
...
Define_Module(AoiCaService)

AoiCaService::AoiCaService() :
...
void AoiCaService::initialize()
{
	...
	mLastLowCamTimestamp = mLastCamTimestamp - artery::simtime_cast(aoi::scLowFrequencyContainerInterval);
...
void AoiCaService::trigger()
{
	Enter_Method("trigger");
	checkTriggeringConditions(simTime());
}

void AoiCaService::indicate(const vanetza::btp::DataIndication& ind, std::unique_ptr<vanetza::UpPacket> packet)
{
	...
	if (cam && cam->validate()) {
		...
		emit(aoi::scSignalCamReceived, &obj);
		mLocalDynamicMap->updateAwareness(obj);
		// Dump the results of the received CAM
		Position current_pos = mVehicleDataProvider->position();
		int latitude = aoi::round(mVehicleDataProvider->latitude(), aoi::microdegree) * Longitude_oneMicrodegreeEast;
		int longitude = aoi::round(mVehicleDataProvider->longitude(), aoi::microdegree) * Latitude_oneMicrodegreeNorth;
...
void AoiCaService::checkTriggeringConditions(const SimTime& T_now)
{
...
void AoiCaService::sendCam(const SimTime& T_now)
{
	...
	if (T_now - mLastLowCamTimestamp >= artery::simtime_cast(aoi::scLowFrequencyContainerInterval)) {
		aoi::addLowFrequencyContainer(cam, par("pathHistoryLength"));
...
SimTime AoiCaService::genCamDcc()
{
